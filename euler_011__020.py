...
def euler11():
	...
	for a in range(20):
		for b in range(17):
			for c in grid[a][b:b+4]: #b dahil b+2 dahil değil
				total_2 = total_2 * c
			else:
				if total_2 < max_product:
					total_2 = 1
					continue
				else:
					max_product = total_2
	...
	for a in range(20):
		for b in range(17):
			if (grid[b][a]*grid[b+1][a]*grid[b+2][a]*grid[b+3][a]) < max_product:
				continue 
			else:
				max_product = grid[b][a]*grid[b+1][a]*grid[b+2][a]*grid[b+3][a]
				
	for a in range(17):
		for b in range(17):
			if (grid[b][a]*grid[b+1][a+1]*grid[b+2][a+2]*grid[b+3][a+3]) < max_product:
				
				continue 
			else:
				
				max_product = grid[b][a]*grid[b+1][a+1]*grid[b+2][a+2]*grid[b+3][a+3]
	...
	print(max_product)
def euler12(): # İŞLEM SÜRESİ 4550 sn. pypy3 ile (1 snde çözülebiliyor)
	#The sequence of triangle numbers is generated by adding the natural numbers. 
	# So the 7th triangle number would be 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28
	# We can see that 28 is the first triangle number to have over five divisors.
	# What is the value of the first triangle number to have over five hundred divisors?
	enyuksekbolen = 1
	for terim_n in range (1, 40000):
		sayi = (terim_n*(terim_n+1))//2
		bln1 = sum(not sayi%(n+1) for n in range(sayi))
		if bln1 > enyuksekbolen:
			enyuksekbolen = bln1

		if bln1 > 500 :
			print('\n \n {}. eleman =  {}. 	Bölen sayısı: {}.	 Sonuç: Olumlu....'.format(terim_n, sayi, bln1))
			break
		else:
			print('\r{}. el =  {}. Bölen say: {}.	Sonuç: [-]  En Yüksek Bölen : {}'.format(terim_n, sayi, bln1, enyuksekbolen),end='')
	else:
		print('sonuca ulasılamadı...')
def euler13():
	...
	print(str(sum(sayi for sayi in sayilar))[:10])
...
def euler14():
	...
	#starting_number = 2
	#chain_length = collatz(starting_number)
	highest = 2
	t = 0
	...
	i = 100
	#def ntrl
	while collatz(i) < 1_000_000:
		if collatz(i) > chain_new:
			chain_new = collatz(i)
			i +=1	
	print(chain_new, i)
	'''
